...
- **Pack system prompts must explicitly guide tool vs picker vs query usage.** Every pack with API access needs all three: a tool for reads the LLM needs to see, a picker for selection lists, and a query component for writes. See `azurePicker`/`githubPicker` as reference implementations. Register intent resolvers for common picker use cases (e.g., `github-orgs`, `azure-regions`).\n- **Picker components must work in BOTH modes.** Intent resolvers only fire in Intent mode. In Adaptive (full-spec) mode, the LLM follows the pack system prompt directly and must emit picker nodes in the layout JSON. The pack system prompt must include full picker component examples with all props so the LLM can generate them. Tool function descriptions must NOT mention listing for selection \u2014 otherwise the LLM will call the tool instead of emitting a picker.
...
- **Pack components must respect `disabled` context.** Past turns render inside a `DisabledScope` where `useAdaptive().disabled === true`. Any `useEffect` that makes API calls must check `if (disabled) return;` at the top. The component still renders visually — only side effects are suppressed.
